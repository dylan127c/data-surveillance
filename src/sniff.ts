...
function envCheck(): string {
    ...
    const reserveDev = "./src/chromium/chrome.exe"; // *.DEVELOP/CHROME-HEADLESS
...
async function init() {
    ...
    if (!browser) {
        browser = await puppeteer.launch({
            ...
            // executablePath: "C:/Program Files/Google/Chrome/Application/chrome.exe", // *.测试用
            defaultViewport: {
                width: 1280,
                height: 720,
            },
...
async function logon(url: string, username: string, passcode: string) {
    ...
    await safelyUse(page, async (page) => {
        ...
        // !.2025/09/24 更新：增加了对登录失败的检查的 timeout 时间（8秒 -> 120秒）
        await page.waitForSelector(params.page.mainSelect, { timeout: 120000 })
            .catch(async (error) => {
...
async function action() {
    try {
        ...
        if (error instanceof Error && error.message !== FROM_TERMINATE) {
...
async function checkMask(times: number[]) {
    // *.2025/09/24 更新：修改等待逻辑，统一等待 MASK_OVERLAY_SELECT 元素消失
    if (times[0] === -1) {
        let loop = true;
        while (loop) {
            await terminateCheck();
            await wait(2000); // *.每两秒检查一次
            try {
                await page?.waitForSelector(MASK_OVERLAY_SELECT, { hidden: true, timeout: 0 });
                loop = false;
            } catch (error) {
                // *.继续等待
            }
